...
export default function Home() {
  ...
  const [location, setLocation] = useState("Tại Siêu Di Động");
  ...
  useEffect(() => {
    ...
    try {
      ...
      setLocation(data.location || "Tại Siêu Di Động");
  ...
  const resolvedLocation =
    location === "Khác..." ? customLocation.trim() || "Tại Siêu Di Động" : location;
      ...
      {showSplash && (
        <div className="brand-splash" aria-hidden="true">
          <div className="brand-splash-inner">
            <img className="brand-splash-symbol" src="/sieu-di-dong-symbol.png" alt="" />
            <img className="brand-splash-wordmark" src="/sieu-di-dong-wordmark.png" alt="" />
      ...
      <aside className={mobileOpen ? "sidebar open" : "sidebar"}>
        <div className="brand-box brand-box-v13">
          ...
          <div className="brand-copy">
            ...
            <small>Siêu Di Động · V13.1</small>
          </div>
          <button
                ...
                <div className="character-controls">
                  <label>
                    ...
                    <select value={buyerType} onChange={(event) => setBuyerType(event.target.value)}>
                      {defaultBuyerTypes.map((item) => <option key={item}>{item}</option>)}
                    </select>
                    {buyerType === "Khác..." && (
                      <input
                        value={customBuyerType}
                        onChange={(event) => setCustomBuyerType(event.target.value)}
                        placeholder="Ví dụ: Bác bảo vệ, chị kế toán..."
                      />
                    )}
                  </label>
                  ...
                  <label>
                    ...
                    <select value={companion} onChange={(event) => setCompanion(event.target.value)}>
                      {companionOptions.map((item) => <option key={item}>{item}</option>)}
                    </select>
                    {companion === "Khác..." && (
                      <input
                        value={customCompanion}
                        onChange={(event) => setCustomCompanion(event.target.value)}
                        placeholder="Nhập người đi cùng..."
                      />
                    )}
                  </label>
                  ...
                  <label>
                    ...
                    {location === "Khác..." && (
                      <input
                        ...
                        placeholder="Ví dụ: Sân trường, nhà máy..."
                      />
                    )}
                  </label>
                  ...
                  <label>
                    ...
                    <select value={hookSpeaker} onChange={(event) => setHookSpeaker(event.target.value)}>
                      {hookSpeakerOptions.map((item) => <option key={item}>{item}</option>)}
                    </select>
                    {hookSpeaker === "Khác..." && (
                      <input
                        value={customHookSpeaker}
                        onChange={(event) => setCustomHookSpeaker(event.target.value)}
                        placeholder="Ai nói câu đầu tiên?"
                      />
                    )}
                  </label>
                  ...
                  <label>
                    ...
                    <select value={openingEmotion} onChange={(event) => setOpeningEmotion(event.target.value)}>
                      {openingEmotionOptions.map((item) => <option key={item}>{item}</option>)}
                    </select>
                  </label>
                </div>
            ...
            <div className="settings-grid">
              ...
              <section className="panel setting-side">
                ...
                <div className="brand-preview-card">
                  ...
                  <p>Logo Siêu Di Động được dùng ở Sidebar và khu vực mở đầu.</p>
                  <div className="brand-preview">
                    <img src="/sieu-di-dong-logo-full.png" alt="Logo Siêu Di Động" />
                  </div>
                </div>
